...
# To fix ambiguities
convert(::Type{Union{T, Missing}}, x::T) where {T} = x
convert(::Type{Union{T, Missing, Nothing}}, x::T) where {T} = x
convert(::Type{Missing}, ::Missing) = missing
convert(::Type{Union{Nothing, Missing}}, x::Union{Nothing, Missing}) = x
convert(::Type{Union{Nothing, Missing, T}}, x) where {T} = convert(T, x)
